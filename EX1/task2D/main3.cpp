...
void initialize_proc_grid(std::vector<double> &proc_grid, int rows, int cols, double fix_east,
                          double fix_west, std::array<int, 2> coords, int n_procs_x, bool is_single_process_mode) {
...
int main(int argc, char *argv[]) try {
    ...
    if (my_rank==0) std::cout << "num_proc_x = " << num_proc_x << std::endl;
    if (my_rank==0) std::cout << "num_proc_y = " << num_proc_y << std::endl;
    ...
    bool has_cols_remainder = N % num_proc_x != 0;
    bool has_rows_remainder = N % num_proc_y != 0;

    if(my_rank==3) std::cout << "My rank " << my_rank << " has coords " << coords[0] << " " << coords[1] << std::endl;
    ...
    if(cart_rank==0) std::cout << "Rows: " << rows_proc << " Cols: " << cols_proc << std::endl;
    ...
    initialize_proc_grid(proc_grid_1, rows_proc, cols_proc, opts.fix_east, opts.fix_west, coords, num_proc_x, is_single_process_mode);
    ...
    for (int iter = 0; iter < opts.iters; ++iter) {
      if (is_single_process_mode) {
        ...
        jacobi_iteration_1D(N, rows_proc, cols_proc, num_proc_y, proc_grid_1, proc_grid_2, cart_rank);
...
/*         std::vector<MPI_Request> requests(4, MPI_REQUEST_NULL);
        ...
        if (cart_rank > 0) {
          MPI_Isend(proc_grid_2.data() + south_boundary_start, n, MPI_DOUBLE, nb[SOUTH], 0, comm_cart, &requests[0]);
        }
        // Recieve in north ghost layer - except for last process
        if (cart_rank < num_proc - 1) {
          MPI_Irecv(proc_grid_2.data() + north_ghost_start, n, MPI_DOUBLE, nb[NORTH], 0, comm_cart, &requests[1]);
        }
        // Send northern boundary - except for last process
        if (cart_rank < num_proc - 1) {
          MPI_Isend(proc_grid_2.data() + north_boundary_start, n, MPI_DOUBLE, nb[NORTH], 0, comm_cart, &requests[2]);
        }
        // Recieve in south ghost layer - except for first process
        if (cart_rank > 0) {
          MPI_Irecv(proc_grid_2.data() + south_ghost_start, n, MPI_DOUBLE, nb[SOUTH], 0, comm_cart, &requests[3]);
        }
    ...
    if (cart_rank == 0 && !is_single_process_mode && is_1D_mode) {
      std::vector<double> merged_grid = merge_subgrids_1D(num_proc, pts_proc, N, proc_grid_1, cols_proc, rows_proc, comm_cart, has_rows_remainder);
      writeToFile(opts.name, merged_grid, opts.N);
    }
    if(is_single_process_mode) writeToFile(opts.name, proc_grid_1, opts.N);
    ...
    if (cart_rank == 0) {
      ...
      std::cout << "time = " << std::chrono::duration<double>(end - start).count() * 1000 << " ms\n" <<std::endl;
        ...
        std::vector<double> merged_grid = merge_subgrids_1D(num_proc, pts_proc, N, proc_grid_1, cols_proc, rows_proc, comm_cart, has_rows_remainder);
        std::cout << "norm2 = " << norm2(merged_grid, opts.N) << std::endl;
        std::cout << "normInf = " << normInf(merged_grid, opts.N) << std::endl;
        std::cout << "time = " << std::chrono::duration<double>(end - start).count() * 1000 << " ms\n" <<std::endl;
      }
      else {
