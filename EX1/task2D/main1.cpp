...
void jacobi_iteration(int N, int rows, int cols, int num_procs_x, int num_procs_y, std::array<int, 2> cart_coords,
                      std::vector<double> &proc_grid_old, std::vector<double> &proc_grid_new, int rank, bool residual) {
  auto h = 1.0 / (N - 1);
  auto h2 = h * h;
  /*
  ...
  for (int j = 0; j < N; ++j) {
    for (int i = 1; i < (N-1); ++i) {
      if (std::abs(x[j + i * N]) > max)
        max = std::fabs(x[i + j * N]) > max ? std::fabs(x[i + j * N]) : max;
    }
  }
  return max;
}
...
int main(int argc, char *argv[]) try {
    ...
    jacobi_iteration(opts.N, rows_proc, cols_proc, num_proc_x, num_proc_y, coords, proc_grid_1, proc_grid_2, cart_rank, true);
