// mpic++ main1.cpp -std=c++17 -O3 -pedantic -march=native -ffast-math -o solverMPI
...
std::vector<double> merge_subgrids(int num_proc, int pts_proc, int global_N, std::vector<double> proc_grid_2,  int cols_proc, int rows_proc, MPI_Comm comm_cart) {
...
int main(int argc, char *argv[]) try {
    ...
    // Adapt N for equal process grids without remainder
    if (opts.N % num_proc != 0) {
      int floor = opts.N / num_proc;
      opts.N = (floor + 1) * num_proc;
      if (my_rank == 0)
        std::cout << "New global N: " << opts.N << std::endl;
    }

    // Local process grid size with ghost layers
    int n_ghost_layers = 2;
    int rows_proc = opts.N / num_proc + n_ghost_layers;
    int cols_proc = opts.N + n_ghost_layers;
    int pts_proc = rows_proc * cols_proc;
    ...
    if (cart_rank == 0) {
      std::vector<double> merged_grid = merge_subgrids(num_proc, pts_proc, opts.N, proc_grid_2, cols_proc, rows_proc, comm_cart);
    ...
    if (cart_rank == 0) {
      std::vector<double> final_grid = merge_subgrids(num_proc, pts_proc, opts.N, proc_grid_2, cols_proc, rows_proc, comm_cart);
