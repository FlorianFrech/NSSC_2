...
int main(int argc, char** argv) {
    ...
    if( grk == 0) {
      std::cout<< "x dimension " << num1 << std::endl;
      std::cout<< "y dimension " << num2 << std::endl;
    }
...
/*     if(coord[0] == 1 && coord[1] == 0) {
        std::cout << "Initialized grid with:" << std::endl;
        std::cout << "coord:\t" << coord[0] << " " << coord[1] << std::endl;
        for(int i =0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                std::cout << x1[j + i * width] << "\t";
            }
            std::cout << std::endl;
        }
    } */
...
/*         // Print grids of processes
        ...
        // Print grids of processes
/*         if (coord[0] == 1 && coord[1] == 0) {
          std::cout << "coord: " << coord[0] << " " << coord[1] << " Iteration: " << iter << std::endl;
          for(int i =0; i < height; i++) {
            for (int j = 0; j < width; j++) {
              std::cout << x1[j + i * width] << "\t";
            }
